...
@Service
public class SendBackOrderExecutor extends AbstractRestBusiExecutor {
    ...
    protected Object executeBusiness(HttpServletRequest request, HttpServletResponse response, BasicModel model, Object... params) throws BusinessException {
        ...
        if(Integer.valueOf(orderList.get(0).get("orderStatus")+"")!=2){
            throw new BusinessException("待发货的订单才可以发货");
        }
        if(CommonUtil.isEmpty(orderList.get(0).get("expressNo"))){
            throw new BusinessException("没填物流信息，不能发货");
        }
        Map<String,Object> parment=orderList.get(0);
        parment.put("userId",tokenUser.getId());
        int i = orderBusiServiceImpl.sendOrder(parment);
        if(i!=1){
            throw new BusinessException("订单详情发货失败");
        }
        return "success";
    }
    ...
    protected ErrorMessage validate(HttpServletRequest request, BasicModel model, Object[] params) {
        ...
        if(CommonUtil.isEmpty(orderDto.getOrderId())){
            errorMessage.rejectNull("orderId",null,"订单ID");
        }
        return errorMessage;
    }
}
