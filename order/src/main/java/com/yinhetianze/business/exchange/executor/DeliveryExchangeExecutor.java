...
@Service
public class DeliveryExchangeExecutor extends AbstractRestBusiExecutor {
    @Autowired
    ...
    protected Object executeBusiness(HttpServletRequest request, HttpServletResponse response, BasicModel model, Object... params) throws BusinessException {
        ...
        exchangePojo.setExpressTime(new Date());
        exchangePojo.setExchangeStatus((short)3);
        int i=exchangeBusiServiceImpl.deliveryExchange(exchangePojo,tokenUser.getId());
        if(i!=1){
            throw new BusinessException("卖家售后发货失败");
        }
        return "success";
    }
    ...
    protected ErrorMessage validate(HttpServletRequest request, BasicModel model, Object[] params) {
        ...
        if(CommonUtil.isEmpty(exchangeModel.getCompanyName())){
            errorMessage.rejectNull("companyName",null,"快递公司");
        }
        if(CommonUtil.isEmpty(exchangeModel.getExpressCode())){
            errorMessage.rejectNull("expressCode",null,"快递单号");
        }
        return errorMessage;
    }
}
