...
@Service
public class TaskGetRequestExecutor extends AbstractClientExecutorGetAdapter
{
    ...
    {
        String serverName = applicationPropertiesUtil.getStrProp("server.task.spring.application.name");
        return serverName;
    }
    ...
    {
        // 记录日志
        LoggerUtil.warn(TaskGetRequestExecutor.class, "[{}] responses failed！failed message ：requestURI={}，params={}",
                new Object[]{applicationPropertiesUtil.getStrProp("server.task.spring.application.name"), request.getRequestURI(), CommonUtil.showRequestParams(request)});
    ...
    {
        return applicationPropertiesUtil.getStrProp("server.task.server.context-path", "/");
    }
}
