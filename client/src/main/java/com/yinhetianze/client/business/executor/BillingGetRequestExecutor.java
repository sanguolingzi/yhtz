...
@Service
public class BillingGetRequestExecutor extends AbstractClientExecutorGetAdapter
{
    ...
    {
        String serverName = applicationPropertiesUtil.getStrProp("server.billing.spring.application.name");
        return serverName;
    }
    ...
    {
        // 记录日志
        LoggerUtil.warn(BillingGetRequestExecutor.class, "[{}] responses failed！failed message ：requestURI={}，params={}",
                new Object[]{applicationPropertiesUtil.getStrProp("server.billing.spring.application.name"), request.getRequestURI(), CommonUtil.showRequestParams(request)});
    ...
    {
        return applicationPropertiesUtil.getStrProp("server.billing.server.context-path", "/");
    }
}
