...
@Service
public class AddCateParamRelaExecutor extends AbstractRestBusiExecutor {
    ...
    protected Object executeBusiness(HttpServletRequest request, HttpServletResponse response, BasicModel model, Object... params) throws BusinessException {
        ...
        BeanUtils.copyProperties(model,cateParamRelaPojo);
        ParameterPojo parameterPojo=new ParameterPojo();
        BeanUtils.copyProperties(cateParamRelaPojo.getParameterPojo(),parameterPojo);
        int results=parameterBusiServiceImpl.addParameterId(parameterPojo);
        if(results<=0)
            throw new BusinessException("BC0050", ResponseConstant.RESULT_DESCRIPTION_FAILED);
        cateParamRelaPojo.setParamId(parameterPojo.getId());
        int result=cateParamRelaBusiServiceImpl.insertSelective(cateParamRelaPojo);
        if(result<=0)
            throw new BusinessException("BC0050", ResponseConstant.RESULT_DESCRIPTION_FAILED);
        return "success";
    }
