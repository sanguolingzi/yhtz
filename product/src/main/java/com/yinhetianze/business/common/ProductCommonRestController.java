...
public class ProductCommonRestController extends RestfulController
{
    /**
     * 文件上传
     * @param request
     ...
     * @return
     */
    @PostMapping (value = "/fileUploadToOss")
    public Object fileUploadToOssExecutor(HttpServletRequest request, HttpServletResponse response
            , SpringBootFileModel springBootFileModel)
    {
        // 执行业务
        BusinessExecutor<String> executor = (BusinessExecutor<String>)ApplicationContextFactory.getBean("fileUploadToOssExecutor");
        return CommonUtil.isNull(executor) ? null : executor.execute(request, response, springBootFileModel);
    }
     ...
     * @return
     */
    @PostMapping (value = "/batchFileUploadToOss")
    public Object batchFileUploadToOssExecutor(HttpServletRequest request, HttpServletResponse response
            , SpringBootFileModel springBootFileModel)
    {
        // 执行业务
        BusinessExecutor<String> executor = (BusinessExecutor<String>)ApplicationContextFactory.getBean("batchFileUploadToOssExecutor");
        return CommonUtil.isNull(executor) ? null : executor.execute(request, response, springBootFileModel);
    }
